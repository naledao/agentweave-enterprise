package com.agentweave.conversation.application;

import com.agentweave.graphrag.application.GraphRagRetrievalService;
import com.agentweave.graphrag.dto.GraphRagRetrievalResponse;
import com.agentweave.springai.rag.QueryRouterService;
import com.agentweave.springai.rag.RagRetrievalPlan;
import com.agentweave.springai.rag.RagRoutingRequest;
import com.agentweave.springai.rag.VectorRetrievalService;
import com.agentweave.springai.rag.dto.VectorRagSearchRequest;
import com.agentweave.springai.rag.dto.VectorRagSearchResponse;
import com.agentweave.springai.rag.dto.VectorRagCitationResponse;
import com.agentweave.shared.security.CurrentUserService;
import com.agentweave.shared.security.CurrentUser;
import java.util.List;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.stereotype.Service;

@Service
public class ConversationRagService {

    private static final Logger log = LoggerFactory.getLogger(ConversationRagService.class);
    private static final String SEARCH_PERMISSION = "knowledge:rag:search";

    private final VectorRetrievalService vectorRetrievalService;
    private final GraphRagRetrievalService graphRagRetrievalService;
    private final RagContextMerger ragContextMerger;
    private final CurrentUserService currentUserService;
    private final QueryRouterService queryRouterService;

    public ConversationRagService(
            VectorRetrievalService vectorRetrievalService,
            GraphRagRetrievalService graphRagRetrievalService,
            RagContextMerger ragContextMerger,
            CurrentUserService currentUserService,
            QueryRouterService queryRouterService) {
        this.vectorRetrievalService = vectorRetrievalService;
        this.graphRagRetrievalService = graphRagRetrievalService;
        this.ragContextMerger = ragContextMerger;
        this.currentUserService = currentUserService;
        this.queryRouterService = queryRouterService;
    }

    public RagPromptContext retrieve(ConversationPrompt prompt) {
        CurrentUser currentUser = currentUserService.getCurrentUser().orElse(null);
        if (currentUser == null || !currentUserService.hasPermission(SEARCH_PERMISSION)) {
            return ragContextMerger.merge(List.of(), List.of());
        }
        RagRetrievalPlan retrievalPlan = queryRouterService.route(new RagRoutingRequest(
                        prompt.latestUserMessage(),
                        prompt.conversationId(),
                        null,
                        null,
                        null,
                        null,
                        currentUser))
                .retrievalPlan();
        try {
            VectorRagSearchResponse vectorResponse = retrievalPlan.shouldSearchVector()
                    ? vectorRetrievalService.search(new VectorRagSearchRequest(
                            prompt.latestUserMessage(),
                            null,
                            null,
                            null,
                            null,
                            null,
                            null,
                            retrievalPlan.vectorTopK(),
                            retrievalPlan.similarityThreshold()),
                            retrievalPlan.retrievalMode())
                    : null;
            GraphRagRetrievalResponse graphResponse = retrievalPlan.shouldSearchGraph()
                    ? graphRagRetrievalService.retrieve(prompt, vectorResponse)
                    : GraphRagRetrievalResponse.empty();
            return ragContextMerger.merge(
                    retrievalPlan.retrievalMode().name(),
                    vectorCitations(vectorResponse),
                    graphResponse.graphPaths());
        } catch (RuntimeException ex) {
            log.warn(
                    "Conversation RAG retrieval failed, falling back to empty context: conversationId={}",
                    prompt.conversationId(),
                    ex);
            return ragContextMerger.merge(List.of(), List.of());
        }
    }

    private List<VectorRagCitationResponse> vectorCitations(VectorRagSearchResponse vectorResponse) {
        if (vectorResponse == null || vectorResponse.citations() == null) {
            return List.of();
        }
        return vectorResponse.citations();
    }
}
